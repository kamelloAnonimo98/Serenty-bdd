 ...
 */
@WithTag("CALCULATOR")
public class OpenCalculatorMultiplicationsProjectTest extends AbstractTest {
    ...
    private MultiplicationsValues multiplicationsValues;
    ...
    public void setup() {
        ...
        multiplicationsValues = taskInstance(MultiplicationsValues.class);
        multiplicationsValues.setFirstValue(config.getFirstValue());
        multiplicationsValues.setSecondValue(config.getSecondValue());
    ...
    public void openCalculatorProjectTest() {
        ...
        if(titleCalculatorExist.answeredBy(actor)){
            then(actor).attemptsTo(multiplicationsValues);
        }
        UserActions.sleep();
        then(actor).should(
              seeThat(resultValueExist,is(ConstantsCalculator.RESULT_MULTIPLICATIONS))
        );
