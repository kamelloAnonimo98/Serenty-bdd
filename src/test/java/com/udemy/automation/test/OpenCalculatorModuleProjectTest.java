...
import com.udemy.automation.application.components.calculator.tasks.ModuleValues;
 ...
 */
@WithTag("CALCULATOR")
public class OpenCalculatorModuleProjectTest extends AbstractTest {
    ...
    private ModuleValues moduleValues;
    ...
    public void setup() {
        ...
        moduleValues = taskInstance(ModuleValues.class);
        moduleValues.setFirstValue(config.getFirstValue());
        moduleValues.setSecondValue(config.getSecondValue());
    ...
    public void openCalculatorProjectTest() {
        ...
        if(titleCalculatorExist.answeredBy(actor)){
            then(actor).attemptsTo(moduleValues);
        }
        UserActions.sleep();
        then(actor).should(
              seeThat(resultValueExist,is(ConstantsCalculator.RESULT_MODULE))
        );
